var _filename, _runmode, addToOut, ast, decreaseIndent, fs, increaseIndent, indent, indent_level, iteration, output, p, parseChildNodes, parseNode, parser, pegjs, removeBlankLines, string_raw_js, sys;
...
process.argv[process.argv.length - 2].substr(0, 2) === "--" ? (_runmode = process.argv[process.argv.length - 2]) : (_runmode = "--convert");
try {
...
parseChildNodes = function(nodes) {
  var _a, _node, i, is_break, is_just_var, is_labelled_statement, is_last_statement;
  i = 0;
  _a = [];
  while (i < nodes.length) {
    _a.push((function() {
      ...
      is_just_var = is_last_statement && _node.type === "Variable";
      is_break = _node.type === "BreakStatement";
      is_labelled_statement = _node.type === "LabelledStatement";
      !is_break && !is_labelled_statement ? indent() : null;
      !is_just_var && !is_break && !is_labelled_statement ? parseNode(_node) : null;
      return addToOut("\n");
    })());
  }
  return _a;
...
parseNode = function(node) {
  var _a, _b, _c, _d, escapedValue, i;
  iteration = iteration + 1;
  if (_runmode === "--debug") {
    sys.puts(iteration + " " + node.type);
    p(node);
  }
  _runmode === "--ilevel" ? sys.puts(iteration + " (" + indent_level + ") " + node.type + " - " + node.name) : null;
  if ((_a = node.type) === "Program") {
    ...
    return !(node.name.substr(0, 3) === "var") ? addToOut(node.name.trim()) : node.name.substr(0, 3) === "var" ? addToOut() : null;
    ...
    if (node.arguments.length > 0) {
      ...
      while (i < node.arguments.length) {
        ...
        i < node.arguments.length - 1 ? addToOut(", ") : null;
      }
    }
    return addToOut(")");
